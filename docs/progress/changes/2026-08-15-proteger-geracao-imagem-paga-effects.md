...
- **RED:** criação de `tests/test_paid_image_effects.py` espelhando `test_paid_voice_effects.py`, cobrindo replay idempotente, reserva e dedução de `openai_image_units`, opt-in de `ORCH_ENABLE_PAID_ADAPTERS`, obrigatoriedade de `PostgresEffectLedger`, classificação de falhas de rede (`ConnectTimeout` vs `ReadTimeout`), bypass para `MockAdapter`, validação de replay não-vazio e transição para `uncertain` em colisão de reserva. Testes estendidos para persistência canônica em R2 e falhas antes de `mark_succeeded` falharam com `TypeError` de argumentos não aceitos por `build_creator_tool`. Teste para falha silenciosa de storage (`put_from_url` -> `None`) falhou inicialmente com `Failed: DID NOT RAISE RuntimeError`.
- **GREEN:** implementação de `is_paid_creator_adapter` em `src/orchestrator/tools/base.py`, envelopamento de `build_creator_tool` em `execute_paid_effect` com persistência canônica embutida em `_build` (`src/orchestrator/tools/creators.py`) e validação estrita que levanta `RuntimeError` para imagens não persistidas, propagação de storage/db/media_root em `node_roster` (`src/orchestrator/nodes/stages.py`), atualização de `DEFAULT_DEV_QUOTAS` em `src/orchestrator/cli.py` e `image-quota` em `scripts/dev-local`. Todos os 14 testes de `test_paid_image_effects.py` e todos os 128 testes da suíte passaram.
...
- `PYTHONPATH=. rtk proxy uv run pytest tests/test_paid_image_effects.py tests/test_paid_voice_effects.py tests/test_paid_video_effects.py tests/test_creator_real.py tests/test_media_store.py tests/test_media_persistence_wiring.py --no-cov` — 128 testes passando sem falhas.
