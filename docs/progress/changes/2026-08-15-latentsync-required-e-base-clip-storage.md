...
- `orchestrator.tools.video`:
  ...
  - `generate_clip_tool`: repassa `stage=stage` para `ctx.adapter.generate_clip` no caminho não durável.
...
- `rtk proxy env PYTHONPATH=. uv run pytest tests/test_latentsync_pipeline.py tests/test_paid_video_effects.py tests/test_media_persistence_wiring.py tests/test_media_store.py tests/test_replicate_video.py tests/test_stages_coverage.py tests/test_progress_docs.py tests/test_cli.py --no-cov` (166 passed)
