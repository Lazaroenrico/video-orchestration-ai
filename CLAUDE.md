...
- **Voice Design direto**:
  - O provider live é `creator_vercel_elevenlabs_design`; `config-mock` e
    `config-staging` continuam offline, determinísticos e com custo zero.
  - Até três previews são persistidos antes do único gate. Aprovação exige candidato
    do mesmo creator; editar `voice_brief` invalida seleção e candidatos anteriores.
  - Em execução durável, Voice Design/TTS exige `ELEVENLABS_API_KEY`,
    `ORCH_ENABLE_PAID_ADAPTERS=true`, `PostgresEffectLedger` e quotas separadas de
    design chars, voice slots e TTS chars. Efeito ambíguo mantém a reserva.
  - Estado e banco guardam somente URI canônica; base64 e signed URL são transitórios.
