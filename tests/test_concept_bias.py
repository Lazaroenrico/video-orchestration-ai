...
@pytest.fixture
def runtime():
    return LanguageRuntime.from_provider("mock", {})


async def test_bias_none_is_backward_compatible(runtime):
    # Sem viés (default) o comportamento é idêntico ao anterior.
    a = await runtime.generate_concepts(offer="serum", n=10, seed="wk1")
    b = await runtime.generate_concepts(offer="serum", n=10, seed="wk1", bias=None)
    assert a == b


async def test_bias_is_deterministic(runtime):
    a = await runtime.generate_concepts(offer="serum", n=20, seed="wk1", bias=["problem"])
    b = await runtime.generate_concepts(offer="serum", n=20, seed="wk1", bias=["problem"])
    assert a == b


async def test_bias_increases_target_style_share(runtime):
    n = 60
    base = await runtime.generate_concepts(offer="serum", n=n, seed="wk1")
    biased = await runtime.generate_concepts(offer="serum", n=n, seed="wk1", bias=["problem"])
